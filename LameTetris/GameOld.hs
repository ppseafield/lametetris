...
movePiece dx dy = if dy > 0
                  then do should <- checkPieceShouldBeSet
                          if should
                            then setPieceAndStartAnew
                            else moveP
                  else do current <- gets currentPiece
                          brd <- gets board
                          let (x, y) = position current
                              ...
                              edgesh = (R.Z :. 1 :. h)
                              gridSlice = extract (R.Z :. gridEdgeX :. 0) edgesh grid
                              boardSlice = extract (R.Z :. edgex :. y) edgesh brd
                              -- Get the slice of the board one position and check them for collisions
                              collision = any id $ P.zipWith (bothJust) (toList boardSlice) (toList grid)
                          when (not collision) moveP
  where
    moveP = do gs <- get
...
-- | Checks if the point exists on the board
isBoardEmpty :: Point -> Game Bool
isBoardEmpty (x, y) = do brd <- gets board
                         case brd ! (R.Z :. x :. y) of
                           Nothing -> return True
                           _ -> return False
